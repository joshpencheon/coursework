...
describe FaceboxHelper do 
  ...
  context 'when the user is logging in for the first time, and session dictates, the welcome message' do
    it 'should be displayed' do
      @user = User.valid.create!
      @user.stubs(:login_count).returns(1)
      session[:welcome?] = true
      
      helper.stubs(:current_user).returns(@user)
      
      helper.show_welcome_message?.should be_true
    end
  end
  
  context '- a facebox:' do
